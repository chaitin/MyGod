...
`AppLayout` 计算所有会话 `unreadCount` 的总和，并在本地动画状态中保存最近一次观察到的总数。渲染时发现总数变化，就条件更新这份状态快照：总数上升时递增动画版本并激活动画，总数下降时不激活；总数持平不会触发新动画。动画结束后清除激活状态。动画版本作为红点的 React `key`，确保连续消息到达时会重建圆点并重新启动 CSS 动画。未读总数变为零时同步清除动画状态，避免红点以后重新出现时重复播放。
